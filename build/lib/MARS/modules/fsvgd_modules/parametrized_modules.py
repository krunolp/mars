...
from MARS.modules.utils import RngKeyMixin
...
class Dense(ParametrizedModule, RngKeyMixin):
    ...
    def __init__(self, input_size: int, output_size: int, rng_key: jax.random.PRNGKey,
                 ...
                 initializer_b: Callable = jax.nn.initializers.constant(0.01), data: Any = None):
        super().__init__()
    ...
    def init_params(self, inplace: bool = True,
        ...
        if self.has_bias:
            if self.initializer_b.__class__ is BiasInitializer:
                param_inits['b'] = self.initializer_b(param_inits['w'], (self.output_size,), rng_key)
            else:
