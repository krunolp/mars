...
from MARS.modules.utils import get_gps, get_gps_jax
...
class GaussianProcessSim(FunctionSimulator):
    ...
    def __init__(self, num_pts: int, input_size: int = 1, output_scale: float = 1.0, length_scale: float = 1.0,
        ...
        x = np.random.uniform(size=(self.num_pts, 1), low=minval, high=maxval).astype(np.float32)
        y = self.mean_fn(x)
...
class GPMetaDataset(FunctionSimulator):
    def __init__(self, dataset, init_seed=hk.PRNGSequence(2), num_input_pts=16, data_dir=None, buffer=0.2):
...
class GPMetaDatasetJax(FunctionSimulator):
    def __init__(self, init_seed, dataset, num_pts=16, data_dir=None, buffer=0.2):
        super().__init__(output_size=1, init_seed=init_seed)
        from MARS.modules.data_modules.regression_datasets import provide_data
        # Getting data
        self.meta_train_data, _, self.meta_test_data = provide_data(dataset=dataset, seed=init_seed, data_dir=data_dir)
        self.num_pts = num_pts
        # Fitting GPs
        self.gps, self.normalization_stats, (mins, maxs) = get_gps_jax(
            self.meta_train_data, init_seed)
        self.input_size = self.normalization_stats['x_mean'].shape[-1]
